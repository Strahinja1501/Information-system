...
Public Class Musterije
    ...
    Public Sub load_data()
        ...
        Dim Sql As String = "Select * FROM KORISNIK WHERE ZAPOSLEN <> 'Da'"
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If TextBox1.Text <> "" And TextBox2.Text <> "" And TextBox3.Text <> "" And TextBox4.Text <> "" And TextBox5.Text <> "" And TextBox6.Text <> "" And TextBox7.Text <> "" Then
            ...
            MessageBox.Show("Uspešno ste uneli novog korisnika.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
    ...
    Private Sub ExecuteDeleteQuery(ByVal query As String, ByVal recordId As Integer)
        ...
        Using connection As New SqlConnection(ConnectionString)
            Dim command As New SqlCommand(query, connection)
            command.Parameters.AddWithValue("@ID_KORISNIKA", recordId)
            Try
                connection.Open()
                Dim result As Integer = command.ExecuteNonQuery()
                If result > 0 Then
                    MessageBox.Show("Mušterija je uspešno obrisana.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Else
                    MessageBox.Show("Izaberite novu mušteriju.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            Catch ex As Exception
                MessageBox.Show("Ne možete obrisati mušteriju ako se nalazi na porudžbenici.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Finally
                connection.Close()
            End Try
        End Using
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        'prvi delete kod koji radi sa greskom koja gasi aplikaciju ako je ID upisan u porudzbenicu
        ...
        Dim query As String = "DELETE FROM KORISNIK WHERE ID_KORISNIKA = @ID_KORISNIKA"
        Dim ID_KORISNIKA As Integer = Integer.Parse(TextBoxID.Text)
        ExecuteDeleteQuery(query, ID_KORISNIKA)
    ...
    Private Sub Button5_Click_1(sender As Object, e As EventArgs) Handles Button5.Click
        ...
        Dim sqlQuery As String = "SELECT * FROM KORISNIK WHERE (IME_KORISNIKA LIKE '%' + @searchTerm + '%' OR PREZIME_KORISNIKA LIKE '%' + @searchTerm + '%') AND ZAPOSLEN <> 'Da'"
